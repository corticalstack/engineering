...
from copy import replace  # Python 3.13+: use copy.replace() instead of dataclasses.replace()
...
ts = datetime.datetime.fromtimestamp(time.time(), tz=datetime.UTC)

# ❌ These are deprecated — they return naive datetimes (no timezone info)
# datetime.datetime.utcnow()               ❌ deprecated
# datetime.datetime.utcfromtimestamp(ts)   ❌ deprecated

# Common patterns
def utcnow() -> datetime.datetime:
    """Always-aware replacement for datetime.utcnow()."""
...
Works with dataclasses, named tuples, `datetime` objects, and any class implementing `__replace__()`.
This standardizes `dataclasses.replace()` — prefer `copy.replace()` for cross-type code.
...
# Also works with datetime, named tuples, etc.
import datetime
ts = datetime.datetime(2026, 1, 1, tzinfo=datetime.UTC)
ts_next_hour = replace(ts, hour=ts.hour + 1)
```

## F-String Improvements (Python 3.12, PEP 701)

```python
# Same quote characters now allowed inside f-string expressions
employee = {"name": "Alice"}
greeting = f"Hello, {employee["name"]}!"   # Previously a SyntaxError

# Backslashes in expressions
words = ["hello", "world"]
joined = f"{'\n'.join(words)}"             # Now works

# Comments in multiline f-string expressions
result = f"""
Data: {
    employee["name"].upper()  # Can comment here now
}
"""
```
...
These stdlib modules are gone — do not use or import them:

`aifc`, `audioop`, `cgi`, `cgitb`, `chunk`, `crypt`, `imghdr`, `mailcap`, `msilib`,
`nis`, `nntplib`, `ossaudiodev`, `pipes`, `sndhdr`, `spwd`, `sunau`, `telnetlib`,
`uu`, `xdrlib`

Also removed: `2to3`, `lib2to3`, `tkinter.tix`.
