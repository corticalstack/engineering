...
select = [
    ...
    "UP",   # pyupgrade (modernize syntax: typing aliases, old Union/Optional, etc.)
    "B",    # flake8-bugbear (common bugs: mutable defaults, assert in prod, etc.)
    ...
    "RUF",  # ruff-specific rules (including RUF100: unused noqa directives)
]
ignore = [
...
"__init__.py" = ["F401"]  # re-exported names

[tool.ruff.lint.isort]
known-first-party = ["mypackage"]

[tool.ruff.format]
# Defaults match Black — no changes needed for most projects
...
uv pip compile pyproject.toml -o requirements.txt  # Export requirements
```
...
pyenv local 3.12.9             # Set for current directory
...
```bash
# Pin dependencies for applications (in pyproject.toml)
# requests==2.31.0  ← avoid, prefer ranges + lockfile

# Use ranges for libraries
# requests>=2.31.0,<3.0.0

# Lock files - uv generates uv.lock automatically
uv lock                        # Generate/update lockfile
uv sync --frozen               # Install exactly from lockfile (CI)

# Update dependencies
uv lock --upgrade              # Update all
uv add requests --upgrade      # Update one package
```
