...
Configure `asyncio_mode = "auto"` in `pyproject.toml` to avoid decorating every async test:

```toml
[tool.pytest.ini_options]
asyncio_mode = "auto"   # All async test functions run with asyncio automatically
```
...
# Async fixture — also works without decoration in auto mode
@pytest.fixture
...
# Test concurrent operations with TaskGroup
...
For `httpx`-based code, use `respx` instead of `responses` or `requests_mock`:
...
# Or use pytest-httpx for a fixture-based approach
@pytest.fixture
def httpx_mock_users(httpx_mock):
...
```toml
# pyproject.toml — recommended pytest + coverage config
[tool.pytest.ini_options]
testpaths = ["tests"]
asyncio_mode = "auto"
addopts = [
