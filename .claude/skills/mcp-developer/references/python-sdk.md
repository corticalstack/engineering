...
```bash
# Preferred: uv (includes CLI and FastMCP high-level API)
...
**FastMCP** (`from mcp.server.fastmcp import FastMCP`) is the high-level decorator API — use it for new servers. The low-level `Server` class is shown below for reference and advanced use.
...
### Streamable HTTP (remote — preferred since `2025-03-26`)
...
if __name__ == "__main__":
    # Standalone (binds to 127.0.0.1:8000 by default)
    mcp.run(transport="streamable-http")

# Mount onto existing FastAPI app
from fastapi import FastAPI
app = FastAPI()
app.mount("/", mcp.streamable_http_app())

# Stateless mode (horizontally scalable, no subscriptions/sampling)
